...
class DriveDataGenerator(image.ImageDataGenerator):
    def __init__(self,
                 ...
                 data_format=None,
                 brighten_range=0):
        super(DriveDataGenerator, self).__init__(featurewise_center=featurewise_center,
    ...
    def random_transform_with_states(self, x, seed=None):
        ...
        if transform_matrix is not None:
            ...
            x = image.apply_affine_transform(x, transform_matrix, img_channel_axis,
                                      fill_mode=self.fill_mode, cval=self.cval) #apply_transform <- original function
    ...
    def __get_indexes(self, index_array):
        ...
        if self.x_prev_states is not None:
            batch_x_images = np.zeros(tuple([self.batch_size] + list(self.x_images.shape)[1:]),
                                      dtype=K.floatx())
            batch_x_prev_states = np.zeros(tuple([self.batch_size] + list(self.x_prev_states.shape)[1:]),
                                           dtype=K.floatx())
        else:
