...
if __name__ == "__main__":
    ...
    parser.add_argument("--training_folder", dest="training_folder", action="store",
                        help="This parameter defines the training data folder")
    ...
    if not training_flag:
        ...
        hybrid_types = []
        if args.hybrid_types:
            tmp_hybrid_types = args.hybrid_types.split(",")
            hybrid_types = []
            for item in tmp_hybrid_types:
                hybrid_types.append(int(item))
        process_list = []
        ...
        if len(hybrid_types) > 0:
            ...
            for index in range(0, len(hybrid_types), n_threads):
                for i in range(n_threads):
                    ...
                    p = Process(target=main, args=(pretrained_model, args.env_params, args.training_folder, training_flag, trained_rotations, hybrid_type))
            ...
            main(pretrained_model, args.env_params, args.training_folder, training_flag)
